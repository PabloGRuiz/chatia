...
async def chat(request: ChatRequest, current_user: dict = Depends(get_current_user)):
    ...
    user_msg = {"role": "user", "content": request.query, "created_at": datetime.datetime.utcnow()}
    asst_msg = {
        "role": "assistant", 
...
async def get_sessions(current_user: dict = Depends(get_current_user)):
    ...
    cursor = db.chat_sessions.find(
        {"user_id": current_user["email"]}, 
        {"messages": {"$slice": 1}, "created_at": 1} # Solo el primer mensaje para el título
    ).sort("created_at", -1)
...
async def get_session(session_id: str, current_user: dict = Depends(get_current_user)):
    db = get_database()
    try:
        session = await db.chat_sessions.find_one({"_id": ObjectId(session_id), "user_id": current_user["email"]})
        if not session:
...
async def delete_session(session_id: str, current_user: dict = Depends(get_current_user)):
    db = get_database()
    try:
        result = await db.chat_sessions.delete_one({"_id": ObjectId(session_id), "user_id": current_user["email"]})
...
from models import TruncateRequest
@router.put("/sessions/{session_id}/truncate")
async def truncate_session(session_id: str, req: TruncateRequest, current_user: dict = Depends(get_current_user)):
    db = get_database()
    try:
        session = await db.chat_sessions.find_one({"_id": ObjectId(session_id), "user_id": current_user["email"]})
        if not session:
        ...
        await db.chat_sessions.update_one(
            ...
            {"$set": {"messages": truncated_messages}}
        )
...
from models import TitleUpdateRequest
@router.put("/sessions/{session_id}/title")
async def update_session_title(session_id: str, req: TitleUpdateRequest, current_user: dict = Depends(get_current_user)):
    db = get_database()
    try:
