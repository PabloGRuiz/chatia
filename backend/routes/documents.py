...
async def upload_documents(
    ...
    for file in files:
        ...
        try:
            doc_metadata = await process_and_index_file(
                file_bytes, file.filename, folder_id
            )
            uploaded_metadata.append(doc_metadata)
        except ValueError as e:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Error en '{file.filename}': {str(e)}",
            )
                ...
                filter=qd_models.Filter(
                    must=[
                        qd_models.FieldCondition(
                            key="filename", match=qd_models.MatchValue(value=filename)
                        ),
                        qd_models.FieldCondition(
...
async def download_document(doc_id: str, inline: bool = False):
    ...
    if inline:
        ...
        return FileResponse(
            file_path, media_type=media_type, content_disposition_type="inline"
        )
