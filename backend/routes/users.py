...
async def update_user_role(user_id: str, role_data: UserUpdateRole, current_user: dict = Depends(get_admin_user)):
    db = get_database()
...
async def change_my_password(pass_data: PasswordChange, current_user: dict = Depends(get_current_user)):
    ...
    await db.users.update_one({"email": current_user["email"]}, {"$set": {"password_hash": new_hash}})
    
    return {"message": "Contraseña actualizada exitosamente"}

@router.put("/{user_id}/reset-password")
async def reset_user_password(user_id: str, pass_data: PasswordReset, current_user: dict = Depends(get_admin_user)):
    db = get_database()
