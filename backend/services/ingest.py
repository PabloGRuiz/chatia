...
def ocr_pdf(file_path: str) -> str:
    try:
        ...
        print(
            "Error: PyMuPDF (pymupdf) o pytesseract no están instalados. No se puede realizar OCR."
        )
        return ""
...
async def process_and_index_file(
    file_bytes: bytes, filename: str, folder_id: str
) -> dict:
    ...
    try:
        if ext == ".pdf":
            ...
            if not text.strip():
                print(
                    "No se pudo extraer texto seleccionable del PDF. Intentando OCR local..."
                )
    ...
    if not text.strip():
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
        raise ValueError(
            "No se pudo extraer texto del archivo. El archivo podría estar vacío, escaneado sin texto seleccionable, o el motor de OCR local no está disponible/configurado."
        )
