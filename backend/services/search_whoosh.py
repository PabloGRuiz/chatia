...
INDEX_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "whoosh_index"
)
...
def index_document_chunks(folder_id: str, filename: str, chunks: list):
    ...
    for idx, chunk in enumerate(chunks):
        ...
        writer.update_document(
            id=chunk_id, folder_id=folder_id, filename=filename, text=chunk
        )
...
def search_keywords(
    ...
    filenames: list = None,
    limit: int = 20,
    markdown_highlights: bool = False,
):
    ix = get_whoosh_index()
