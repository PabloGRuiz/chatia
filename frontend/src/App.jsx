  ...
  for (let idx = 0; idx < lines.length; idx++) {
    ...
    if (isBullet) {
      renderedElements.push(
        ...
        >
          <span style={{ color: "var(--primary-color)", userSelect: "none" }}>
            •
          </span>
          <span>{parts}</span>
        </div>,
      ...
      renderedElements.push(<blockquote key={idx}>{parts}</blockquote>);
    } else {
      renderedElements.push(
        <div
          key={idx}
          style={{ margin: "6px 0", minHeight: "1em", lineHeight: "1.5" }}
        >
...
function App() {
  ...
  const [theme, setTheme] = useState(
    () => localStorage.getItem("chatia_theme") || "light",
  );
  ...
  const [messages, setMessages] = useState([
    {
      ...
      role: "assistant",
      content:
        "Hola. Soy tu Asistente Jurídico Inteligente. Selecciona una carpeta a la izquierda si deseas acotar la consulta, y escribe tu duda aquí.",
    },
  ]);
  ...
  const fetchDocuments = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/folders/${activeFolder.id}/documents`,
      );
      if (res.ok) {
        const data = await res.json();
        setDocuments(data);
  ...
  const handleUpdateSessionTitle = async (e, sessionId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/chat/sessions/${sessionId}/title`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        setChatSessions((prev) =>
          prev.map((s) =>
            s.id === sessionId ? { ...s, title: editSessionTitle } : s,
          ),
        );
      }
    } catch (err) {
  ...
  const handleAddIntentPhrase = (e) => {
    ...
    if (
      intentPhrases.some(
        (x) => x.toLowerCase() === newIntentInput.trim().toLowerCase(),
      )
    ) {
  ...
  const handleResetIntents = async () => {
    if (
      !confirm(
        "¿Seguro que deseas restablecer el detector a las frases por defecto del sistema? Se eliminarán tus cambios personalizados.",
      )
    )
      return;
    setSavingIntents(true);
  ...
  const handleSemanticSearch = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSearchError(
          errData.detail || "Error al realizar la búsqueda semántica",
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleResetUserPassword = async (id) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/users/${id}/reset-password`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:8000/users/me/password", {
        ...
        body: JSON.stringify({
          old_password: passwordChangeOld,
    ...
    try {
      ...
      if (error.name === "AbortError") {
        ...
        const errorMsg = {
          ...
          role: "assistant",
          content:
            "Error al procesar consulta. Verifica que el backend esté disponible.",
        };
  ...
  const handleEditMessage = async (msgIndex, text) => {
    ...
    if (
      !confirm(
        "Editar este mensaje reescribirá el historial a partir de este punto. ¿Deseas continuar?",
      )
    )
      return;
    ...
    try {
      await fetch(
        `http://localhost:8000/chat/sessions/${currentSessionId}/truncate`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteFolder = async (id) => {
    if (
      !confirm(
        "¿Está seguro de que desea eliminar esta carpeta? Se borrarán todos sus documentos e índices vectoriales.",
      )
    )
      return;
    try {
  ...
  const handleUploadDocument = async (e) => {
    ...
    if (totalSize > MAX_TOTAL_SIZE) {
      alert(
        `El tamaño total de los archivos (${(totalSize / (1024 * 1024)).toFixed(2)} MB) excede el límite máximo permitido de 100 MB.`,
      );
      return;
    }
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/folders/${activeFolder.id}/documents/upload`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${user.token}` },
          body: formData,
      ...
      <div className="login-screen">
        <form className="login-card" onSubmit={handleLogin}>
          ...
          <p className="login-subtitle">
            Departamento de Planeamiento - Ejército Argentino
          </p>
          ...
          <div className="form-group">
            <label className="form-label">
              <Mail size={12} style={{ marginRight: 4 }} /> Correo electrónico
            </label>
            <input
              type="email"
          ...
          <div className="form-group">
            <label className="form-label">
              <Lock size={12} style={{ marginRight: 4 }} /> Contraseña
            </label>
            <input
              type="password"
              className="form-input"
    ...
    <div className="app-container">
      ...
      <div className="topbar">
        <div className="topbar-left">
          <img
            src="/logo.png"
            alt="Ejército Argentino Logo"
            style={{ height: "38px", width: "auto", objectFit: "contain" }}
          />
          <div>
            <span className="topbar-title">
              Asistente IA - Ejercito Argentino
            </span>
          ...
          <>
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    fontWeight: sidebarView === "folders" ? 600 : 400,
                  }}
                >
                  Carpetas
                </div>
                  ...
                  style={{
                    ...
                    color:
                      sidebarView === "history"
                        ? "var(--primary-color)"
                        : "rgba(255,255,255,0.7)",
                    fontWeight: sidebarView === "history" ? 600 : 400,
                  }}
                >
                  Historial
                </div>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Folder
                              size={16}
                              fill={isFolderActive ? "currentColor" : "none"}
                            />
                            <span
                              style={{ fontWeight: isFolderActive ? 600 : 400 }}
                            >
                              {folder.name}
                            </span>
                          </div>
                          ...
                          >
                            {docsByFolder[folder.id] === undefined ? (
                              ...
                              docsByFolder[folder.id].map((doc) => {
                                const isDocActive =
                                  activeFolder?.id === folder.id &&
                                  selectedDocsForChat.includes(doc.filename);
                                return (
                                  <div
                                    key={doc.id}
                                    onClick={(e) => {
                      ...
                      >
                        ...
                        >
                          {editingSessionId === session.id ? (
                            <form
                              onSubmit={(e) =>
                                handleUpdateSessionTitle(e, session.id)
                              }
                        ...
                        <button
                          onClick={(e) =>
                            handleDeleteChatSession(e, session.id)
                          }
                          style={{
                            background: "transparent",
                            border: "none",
            ...
            <div className="chat-area">
              <div className="messages-container">
                  ...
                  <div key={msg.id} className={`message ${msg.role}`}>
                        ...
                        {(msg.folder_name || msg.execution_time) && (
                          ...
                          >
                            {msg.folder_name && (
                              ...
                              >
                                <Database size={10} /> Consultado en:{" "}
                                {msg.folder_name}{" "}
                                {msg.filenames && msg.filenames.length > 0
                                  ? `> ${msg.filenames.length} archivo(s)`
                                  : ""}
                              </div>
                            )}
                            {msg.execution_time !== undefined &&
                              msg.execution_time !== null && (
                                <div
                                  style={{
                                    display: "flex",
                                    alignItems: "center",
                                ...
                                >
                                  <Clock size={10} /> Respuesta generada en{" "}
                                  {msg.execution_time} segundos
                                </div>
                              )}
                          </div>
                        )}
                    ...
                    {msg.role === "assistant" &&
                      ...
                      msg.source_documents.length > 0 && (
                        <div
                          className="source-docs-container"
                          style={{
                            marginTop: "12px",
                            paddingTop: "10px",
                ...
                {chatLoading && (
                  <div className="message assistant">
                    ...
                    >
                      <Bot size={18} />{" "}
                      <em>Buscando normativas y redactando respuesta...</em>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: "0 20px" }}>
                ...
                >
                  Tema de Consulta actual:{" "}
                  <strong style={{ color: "var(--primary-color)" }}>
                    {activeFolder ? activeFolder.name : "General"}{" "}
                    {selectedDocsForChat.length > 0
                      ? `> ${selectedDocsForChat.length} documento(s)`
                      : ""}
                  </strong>
                </div>
              </div>
              <div className="input-container">
                <div className="input-box">
                  ...
                  <button
                    ...
                    title={
                      chatLoading ? "Detener ejecución" : "Enviar consulta"
                    }
                  >
        ...
        {activeTab === "search" && user.role === "admin" && (
          <div className="admin-container">
            ...
            >
              ...
              >
                Busca en todo el sistema utilizando lenguaje natural. La
                inteligencia artificial convertirá tu consulta en vectores para
                encontrar los documentos más relevantes por coincidencia
                semántica, mostrando el porcentaje de similitud y sus etiquetas.
              </p>
              ...
              >
                {searching ? (
                  ...
                  >
                    <em style={{ fontSize: "0.95rem" }}>
                      Buscando coincidencias semánticas en Qdrant...
                    </em>
                  </div>
                ) : searchResults.length > 0 ? (
                  searchResults.map((result, idx) => {
                    const matchedFolder = folders.find(
                      (f) => f.id === result.folder_id,
                    );
                    return (
                      <div
                        key={idx}
                          ...
                          {result.id && (
                            <div className="search-buttons-wrapper">
                              <button
                                onClick={() =>
                                  window.open(
                                    `http://localhost:8000/folders/documents/${result.id}/download?inline=true`,
                                    "_blank",
                                  )
                                }
                              ...
                              <button
                                onClick={() =>
                                  window.open(
                                    `http://localhost:8000/folders/documents/${result.id}/download`,
                                    "_blank",
                                  )
                                }
                  ...
                  >
                    No se encontraron documentos con coincidencia semántica para
                    "{searchQuery}".
                  </div>
                ) : (
                  <div
                  ...
                  >
                    Ingresa una consulta arriba para empezar a buscar documentos
        ...
        {activeTab === "administracion" && user.role === "admin" && (
          <div className="admin-layout">
            ...
            <div className="admin-card" style={{ flex: 1.5 }}>
              ...
              >
                <FileText size={20} /> Documentos de:{" "}
              ...
              {activeFolder ? (
                <>
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        Subir PDF, Word o TXT (Máx 100MB en total)
                      </label>
                      <input
                        id="file-input"
                        type="file"
                          ...
                          >
                            ...
                            >
                              ...
                              <div style={{ flex: 1 }}>
                                ...
                                >
                                  {doc.chunk_count} fragmentos vectorizados
                                </span>
                                {doc.tags && doc.tags.length > 0 && (
                                  <div
                                    style={{
                        ...
                        {filteredDocuments.length === 0 && (
                          ...
                          >
                            {docSearchQuery
                              ...
                              : "No hay documentos cargados en esta carpeta."}
                          </div>
                        )}
                ...
                >
                  Selecciona o crea una carpeta del listado de la izquierda para
                ...
                >
                  <thead>
                    <tr
                      style={{
                        borderBottom:
                          "2px solid var(--admin-table-header-border)",
                        transition: "border-color 0.3s",
                      }}
                    >
                  ...
                  <tbody>
                    {systemUsers.map((sysUser) => (
                      <tr
                        ...
                        style={{
                          borderBottom:
                            "1px solid var(--admin-table-row-border)",
                          transition: "border-color 0.3s",
                        }}
                      >
                        ...
                        <td style={{ padding: "12px" }}>
                          <select
                            value={sysUser.role}
                            onChange={(e) =>
                              handleChangeRole(sysUser.id, e.target.value)
                            }
        ...
        {activeTab === "glosario" && user.role === "admin" && (
          <div className="admin-container">
            ...
            >
              ...
              >
                Define aquí términos, siglas, fechas clave o doctrina general.
                La Inteligencia Artificial leerá este texto{" "}
                <strong>antes</strong> de responder cualquier consulta, lo que
                ayuda a evitar alucinaciones y mejora la precisión de conceptos
                militares específicos independientemente del documento
                consultado.
              </p>

              <form
                onSubmit={handleSaveGlossary}
                style={{
                  display: "flex",
                  flexDirection: "column",
                  gap: "15px",
                ...
                {glossaryMessage && (
                  <div
                    style={{
                      padding: "10px",
                      borderRadius: "6px",
                      background: glossaryMessage.includes("Error")
                        ? "rgba(239, 68, 68, 0.1)"
                        : "rgba(34, 197, 94, 0.1)",
                      color: glossaryMessage.includes("Error")
                        ? "#ef4444"
                        : "#22c55e",
                    }}
                  >
                ...
                <div style={{ display: "flex", justifyContent: "flex-end" }}>
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "6px",
                      fontWeight: 600,
                      cursor: savingGlossary ? "not-allowed" : "pointer",
                      display: "flex",
                      alignItems: "center",
                      gap: "8px",
                    }}
        ...
        {activeTab === "intents" && user.role === "admin" && (
          <div className="admin-container">
            ...
            >
              ...
              >
                Define aquí frases simples o expresiones regulares (regex)
                avanzadas que activarán el cortocircuito del chat. Cuando una
                consulta coincida con cualquiera de estos patrones, la IA
                buscará los documentos directamente y los devolverá al instante
                sin generar texto nuevo, ahorrando tokens.
              </p>
              ...
              {intentsMessage && (
                <div
                  style={{
                    padding: "10px",
                    borderRadius: "6px",
                    background: intentsMessage.includes("Error")
                      ? "rgba(239, 68, 68, 0.1)"
                      : "rgba(34, 197, 94, 0.1)",
                    color: intentsMessage.includes("Error")
                      ? "#ef4444"
                      : "#22c55e",
                    marginBottom: "20px",
                  }}
              ...
              <form
                onSubmit={handleAddIntentPhrase}
              ...
              >
                {intentPhrases.length === 0 ? (
                  ...
                  >
                    No hay frases personalizadas registradas. Se usarán las
                    frases por defecto del sistema.
                  </div>
                ) : (
                  <div
                    style={{
                    ...
                    {intentPhrases.map((phrase, idx) => (
                      ...
                      >
                        <span
                          style={{
                            fontFamily:
                              phrase.startsWith("\\b") || phrase.includes("\\s")
                                ? "monospace"
                                : "inherit",
                            wordBreak: "break-all",
                          }}
              ...
              >
                ...
                >
                  Nota: Las frases que contienen caracteres especiales (como \,
                  \b, *, +, ?) se interpretan como regex avanzadas.
                </span>
                <div style={{ display: "flex", gap: "10px" }}>
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "6px",
                      fontWeight: 600,
                      cursor: savingIntents ? "not-allowed" : "pointer",
                      transition: "all 0.2s",
                    }}
                    onMouseEnter={(e) => {
                      if (!savingIntents)
                ...
                {passwordChangeMessage && (
                  <div
                    style={{
                      padding: "10px",
                      borderRadius: "6px",
                      background: passwordChangeMessage.includes("Error")
                        ? "rgba(239, 68, 68, 0.1)"
                        : "rgba(34, 197, 94, 0.1)",
                      color: passwordChangeMessage.includes("Error")
                        ? "#ef4444"
                        : "#22c55e",
                    }}
                  >
